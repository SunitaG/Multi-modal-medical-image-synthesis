...
class DRIT(nn.Module):
    ...
    def forward(self):
        ...
        if self.ms:
            ...
            self.fake_A_encoded_s, self.fake_AA_encoded_s, self.fake_A_random_s, _ = torch.split(output_fakeA_s,
                                                                                                 self.z_content_a.size(
                                                                                                     0), dim=0)
            self.fake_B_encoded_s, self.fake_BB_encoded_s, self.fake_B_random_s, _ = torch.split(output_fakeB_s,
                                                                                                 self.z_content_a.size(
                                                                                                     0), dim=0)
        else:
            ...
            self.fake_B_encoded, self.fake_BB_encoded, self.fake_B_random = torch.split(output_fakeB,
                                                                                        self.z_content_a.size(0), dim=0)
            self.fake_A_encoded_s, self.fake_AA_encoded_s, self.fake_A_random_s = torch.split(output_fakeA_s,
                                                                                              self.z_content_a.size(0),
                                                                                              dim=0)
            self.fake_B_encoded_s, self.fake_BB_encoded_s, self.fake_B_random_s = torch.split(output_fakeB_s,
                                                                                              self.z_content_a.size(0),
                                                                                              dim=0)
